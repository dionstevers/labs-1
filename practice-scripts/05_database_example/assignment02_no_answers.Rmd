...
library(tidyverse)
library(DBI)
con <- DBI::dbConnect(RSQLite::SQLite(), "levant.cameo.sqlite3")
```
...
```{r}
event <- tbl(con, "event")
description <- tbl(con, "description")
score <- tbl(con, "score")
```
...
```{r}
begin_date <- "2000-09-27"
end_date <- "2005-02-08"

int_event <- score_events |> 
...
```{r}
real_int_event <- int_event |>  collect()
```
...
```{r}
pal_isr <- real_int_event |> 
  filter(str_starts(source, "ISR"),  #alternative: grepl("ISR", source)
         str_starts(target, "PAL"))  #alternative: grepl("PAL", target)
```
...
```{r}
pal_isr_weekly <- pal_isr |> 
  group_by(week) |> 
  summarise(avg_score = round(mean(score, na.rm=TRUE)))
head(pal_isr_weekly)
```
...
```{r}
ggplot(pal_isr_weekly, aes(x = week, y = avg_score)) + 
